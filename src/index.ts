...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  const output = await runAgent(group, prompt, chatJid, async (result) => {
    ...
    if (result.result) {
      ...
      if (text) {
        const withModel = `${text}\n\n— ${modelLabel(currentModel)}`;
        await channel.sendMessage(chatJid, withModel);
...
async function main(): Promise<void> {
  ...
  // Skip for rootless Docker — containers can't reach the host namespace.
  const proxyServer = isRootlessDocker()
    ? null
  ...
  startSchedulerLoop({
    ...
    sendMessage: async (jid, rawText) => {
      ...
      if (text) {
        const withModel = `${text}\n\n— ${modelLabel(currentModel)}`;
        await channel.sendMessage(jid, withModel);
      }
    },
  });
  startIpcWatcher({
